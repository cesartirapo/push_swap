...
static void	set_target_a(t_node *a, t_node *b)//Find `a` node's target in stack `b`
{
	...
	t_node	*target; //To store the pointer to the target node in stack `b`
	...
	while (a) //As long as we have nodes in stack `a`
	{
		...
		while (current_b) //Iteratively search through all the nodes in stack `b` until the end of the stack is reached
		{
				...
				&& current_b->value > best_match_index) //Check if `b` node is smaller than `a` node && bigger than the closest smaller number so far
			{
				...
				target = current_b; //Assign the current `b` node as the `target_node`
			}
		...
		if (best_match_index == LONG_MIN) //Check if the LONG_MIN hasn't changed, it means we haven't found anything smaller
			...
			a->target = target; //If no "closer smaller number" is found, and the best match has changed
...
static void	cost_analysis_a(t_node *a, t_node *b) //Define a functio that analyses the cost of the `a` node along with it's target `b` node, which is the sum of the number of instructions for both the nodes to rotate to the top of their stacks
{
	...
	while (a) //Loop through each node until the end of the stack is reached
	{
		...
		if (a->target->above_median) //Check if `a` node's target node `b` has a "true" above median attribute, meaning the target `b` node is above median
			a->push_cost += a->target->index; //If so, update `a` node's push cost, the sum of (its current index) + (its target `b` node's index)
		else //If `a` node is indeed above median and its target `b` node is below median
			a->push_cost += len_b - (a->target->index); //Update `a` node's push cost, the sum of (its current index) + (`b` stack's length - its target `b` node's index)
		a = a->next; //Move to the next `a` node for its cost analysis
	}
}

void	set_cheapest(t_node *stack) //Define a function that sets a node's `cheapest` attribute as `true` or `false`
{
